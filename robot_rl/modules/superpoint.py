...
def sample_descriptors(keypoints: torch.Tensor, descriptors: torch.Tensor, s: int = 8) -> torch.Tensor:
...
def batched_nms(scores: torch.Tensor, nms_radius: int) -> torch.Tensor:
    assert nms_radius >= 0
...
def select_top_k_keypoints(keypoints: torch.Tensor, scores: torch.Tensor, k: int) -> tuple[torch.Tensor, torch.Tensor]:
    if k >= len(keypoints):
...
class SuperPoint(nn.Module):
    ...
    def __init__(self, **conf: dict[str, Any]) -> None:
        ...
        for i, c in enumerate(channels[1:], 1):
            layers: list[nn.Module] = [VGGBlock(channels[i - 1], c, 3), VGGBlock(c, c, 3)]
            if i < len(channels) - 1:
